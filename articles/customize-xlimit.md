...
The interactive AE forest plots include AE-specific tables. In this
vignette, we demonstrate how users can customize the limits when
plotting AE proportions, their differences, and corresponding confidence
intervals.
...
Users can control these limits by specifying the `prop_range = ...` and
`diff_range = ...` arguments within the
[`format_ae_forestly()`](https://merck.github.io/forestly/reference/format_ae_forestly.md)
function. Specifically, `prop_range` sets the limits for AE proportions,
while `diff_range` defines the limits for AE proportion differences and
their confidence intervals.

By default, the limits are set based on the minimum and maximum values
observed across all AEs, rounded and extended by ±2 units for better
visualization.

In the example below, we modify the AE proportion limits to range from
-0.5 to 30, and the AE proportion difference limits from -10 to 35.
Please note that if the specified range does not fully encompass the
