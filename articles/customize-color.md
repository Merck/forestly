...
The interactive AE forest plots include AE-specific tables that
visualize AE proportions, differences, and confidence intervals (CI). In
this vignette, we introduce how to customize the plotting colors.
...
Users can specify custom colors by using the `color = ...` argument
within the
[`format_ae_forestly()`](https://merck.github.io/forestly/reference/format_ae_forestly.md)
function. By default, the color scheme is based on the branding of Merck
& Co., Inc., Rahway, NJ, USA.

In the example below, we modify the colors to
`c("black", "grey60", "grey40")` to represent the three arms in the
plot. The default color palette supports up to 4 arms; for studies with
more than 4 arms, we recommend users define their own color settings to
